<!--
Auteur: Anouk Jorritsma, Floris Timmermans
Aanmaakdatum: 20-3-2025

Inhoud: Footer include.
Versie: 1.0
-->

<footer class="footer">
    <div class="footer-columns">
        <div class="footer-column">
            <!--  Shop and learn collumn          -->
            <h4>Shop and Learn</h4>
            <ul>
                <li><a href="https://www.apple.com/store">Store</a></li>
                <li><a href="https://www.apple.com/mac/">Mac</a></li>
                <li><a href="https://www.apple.com/ipad/">iPad</a></li>
                <li><a href="https://www.apple.com/iphone/">iPhone</a></li>
                <li><a href="https://www.apple.com/watch/">Watch</a></li>
            </ul>
        </div>
        <!--  Apple wallet column-->
        <div class="footer-column">
            <h4>Apple Wallet</h4>
            <ul>
                <li><a href="https://www.apple.com/apple-card/">Apple Card</a></li>
                <li><a href="https://www.apple.com/apple-pay/">Apple Pay</a></li>
                <li><a href="https://www.apple.com/wallet/">Wallet</a></li>
            </ul>
        </div>
        <!--   Account column     -->
        <div class="footer-column">
            <h4>Account</h4>
            <ul>
                <li><a href="https://account.apple.com/">Apple Account</a></li>
                <li><a href="https://secure6.store.apple.com/shop/signIn/account?ssi=1AAABlbqR-HAgZZbYMdn6p5T85JB2Qf0BCWrYHoT7VBTPIsaOIii8ZWYAAAAzaHR0cHM6Ly9zZWN1cmU2LnN0b3JlLmFwcGxlLmNvbS9zaG9wL2FjY291bnQvaG9tZXx8AAIBCJxfbtlylJO8qDMPn6RObfCFMY8YhNZbwi5Lqz_RnMs">Apple Store Account</a></li>
            </ul>
        </div>
        <!--    Apple Store column    -->
        <div class="footer-column">
            <h4>Apple Store</h4>
            <ul>
                <li><a href="https://www.apple.com/retail/">Find a Store</a></li>
                <li><a href="https://www.apple.com/today/">Today at Apple</a></li>
            </ul>
        </div>
    </div>
    <!--  Copyright column   -->
    <div class="footer-bottom">
        Copyright &copy; <span class="jaarTal"></span> Apple Inc. All rights reserved.
    </div>
</footer>